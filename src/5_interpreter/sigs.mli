...
module type Stack = sig
    ...
    (** Pops some bytes. *)
    val pop_bytes : t -> Theory.Bytes.t * Dtyp.t

    (** Pops a key hash. *)
    val pop_key_hash : t -> Theory.KeyH.t * Dtyp.t

    (** Pops an optional key hash. *)
    val pop_key_hash_option : t -> Theory.KeyH.t option * Dtyp.t

    (** Pops a mutez value. *)
    val pop_tez : t -> Theory.Tez.t * Dtyp.t

    (** Pops a comparable value. *)
    val pop_cmp : t -> Theory.Cmp.t * Dtyp.t
    ...
    (** Pops a set. *)
    val pop_set : t -> Theory.Set.t * Dtyp.t

    (** Pops a map. *)
    val pop_map : t -> Theory.value Theory.Map.t * Dtyp.t

    (** Pops a pair. *)
    val pop_pair : t -> (Theory.value * Dtyp.t) * (Theory.value * Dtyp.t)

    (** Pops a lambda. *)
    val pop_lambda : t -> Dtyp.t * Dtyp.t * Mic.t
    ...
    *)
    ...
    val pop_contract_params_and_lambda :
        Theory.Address.t -> t -> Theory.contract_params * Mic.contract

    (** Pops parameters for an account creation.

        The difference with `pop_contract_params` is that this one does not retrieve the
        `spendable` flag and initial storage value from the stack. They are set to `true` and
        `unit`.
    *)
    val pop_account_params : Theory.Address.t -> t -> Theory.contract_params
